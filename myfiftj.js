(() => 
{ // protect the lemmings!
    const config = 
    {
        apiKey: "AIzaSyBdN76fdS0vIOWyots_5YNcbvuk1WbG2aE",
        authDomain: "howdy-27dd6.firebaseapp.com",
        databaseURL: "https://howdy-27dd6.firebaseio.com",
        projectId: "howdy-27dd6",
        storageBucket: "howdy-27dd6.appspot.com",
        messagingSenderId: "310611162979",
        appId: "1:310611162979:web:cf1cbccd16787f6e48c556",
        measurementId: "G-5EB12TGK2N"
    };
    firebase.initializeApp(config);
        
    // Get a reference to the storage service, which is used to create references in your storage bucket
    const storage = firebase.storage();

    // Create a storage reference from our storage service
    const storageRef = storage.ref();
        
    // Create a child reference
    var id = Math.floor(Math.random() * (100000000000 - 0000000 + 1)) + 0000000;;
    document.getElementById("demo").innerHTML = id;

    const imagesRef = storageRef.child('multiple-image-' + id);
    // imagesRef now points to 'images'

    // select anchor tag and file input
    const fileSelect0 = document.querySelector('.js-fileSelect0');
    const fileElem0 = document.querySelector('.js-fileElem0');
 	const fileSelect1 = document.querySelector('.js-fileSelect1');
    const fileElem1 = document.querySelector('.js-fileElem1');
    const fileSelect2 = document.querySelector('.js-fileSelect2');
    const fileElem2= document.querySelector('.js-fileElem2');
 	const fileSelect3 = document.querySelector('.js-fileSelect3');
    const fileElem3 = document.querySelector('.js-fileElem3');
    const fileSelect4 = document.querySelector('.js-fileSelect4');
    const fileElem4 = document.querySelector('.js-fileElem4');
 	const fileSelect5 = document.querySelector('.js-fileSelect5');
    const fileElem5 = document.querySelector('.js-fileElem5');
    const fileSelect6 = document.querySelector('.js-fileSelect6');
    const fileElem6 = document.querySelector('.js-fileElem6');
 	const fileSelect7 = document.querySelector('.js-fileSelect7');
    const fileElem7 = document.querySelector('.js-fileElem7');
    const fileSelect8 = document.querySelector('.js-fileSelect8');
    const fileElem8 = document.querySelector('.js-fileElem8');
 	const fileSelect9 = document.querySelector('.js-fileSelect9');
    const fileElem9 = document.querySelector('.js-fileElem9');

    // click handler for fileElem
    fileSelect0.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem0) 
        {
            fileElem0.click();
        }
    });

    // change handler for fileSelect
    fileElem0.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-0'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
fileSelect1.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem1) 
        {
            fileElem1.click();
        }
    });
       fileElem1.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-1'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect2.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem2) 
        {
            fileElem2.click();
        }
    });
       fileElem2.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-2'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect3.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem3) 
        {
            fileElem3.click();
        }
    });
       fileElem3.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-3'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect4.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem4) 
        {
            fileElem4.click();
        }
    });
       fileElem4.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-4'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect5.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem5) 
        {
            fileElem5.click();
        }
    });
       fileElem5.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-5'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect6.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem6) 
        {
            fileElem6.click();
        }
    });
       fileElem6.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-6'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect7.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem7) 
        {
            fileElem7.click();
        }
    });
       fileElem7.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-7'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect8.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem8) 
        {
            fileElem8.click();
        }
    });
       fileElem8.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-8'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       fileSelect9.addEventListener('click', (e) => 
    {
        e.preventDefault();

        // trigger click on input type="file"
        // this will call the change event defined below
        if (fileElem9) 
        {
            fileElem9.click();
        }
    });
       fileElem9.addEventListener('change', (e) => 
    {
        // e.target.files contains File object references
        // to all chosen items by user
        console.log(e.target.files);

        // since e.target.files is "array-like", we turn it into an array
        // then map it to the .put() method from Firebase, which returns promises...
        const fileUploads = Array.from(e.target.files).map((currFile) => 
        {
        // we store the name of the file as a storage ref
            const fileRef = imagesRef.child(currFile.name);
            // we return a promise where we first "put" or upload the file
            // and then once the upload is complete, we return promise with
            // download URL string of the file we uploaded
            return fileRef.put(currFile).then((snapshot) => snapshot.downloadURL);
        });
        var updates = {};
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(position) {
                
 		var lat= position.coords.longitude
 		var lon = position.coords.latitude
        var user_id;
        var month = new Array();
        var weekday = new Array(7);
		weekday[0] = "Sunday";weekday[1] = "Monday";weekday[2] = "Tuesday";weekday[3] = "Wednesday";weekday[4] = "Thursday";weekday[5] = "Friday";weekday[6] = "Saturday";
		month[0] = "January";month[1] = "February";month[2] = "March";month[3] = "April";month[4] = "May";month[5] = "June";month[6] = "July";month[7] = "August";month[8] = "September";month[9] = "October";month[10] = "November";month[11] = "December";
        var d = new Date();;
        var tp = (weekday[d.getDay()] +', ' + d.getDate() + ' ' + month[d.getMonth()] + ' ' + d.getFullYear() + ', ' + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds() + ' (IST)' );
        user_id = user_id ? user_id : firebase.database().ref().child('multiple image-test' + id).push().key;
        var postData = 
        {
            Application_ID : id,
            Latitude : lat,
            Longitude: lon,
            TimeStamp: tp
        }
        updates['/POST-9'] = postData;
        firebase.database().ref().child('multiple-image-test' +id ).update(updates);
        // once ALL the promises have been resolved
        // we console.log the urls...but we can do whatever we need to with this data 
        // from here
        Promise.all(fileUploads).then((items) => 
        {
            console.log(items);
        });   
                   });
        } 
       
    });
       
})();


